	    ...
	    if ( $show == 'logged_out' ) {
		    if ( get_option('users_can_register') && ! empty( $this->instance['show_register_link'] ) && $this->instance['show_register_link'] == 1 ) {
		    	...
		    	if ( ! is_multisite() ) {
		    		...
		    		$links['register'] = array(
		    			'text' => __( 'Регистрация', 'sidebar-login' ),
		    ...
		    if ( ! empty( $this->instance['show_lost_password_link'] ) && $this->instance['show_lost_password_link'] == 1 ) {
		    	...
		    	$links['lost_password'] = array(
	    			'text' => __( 'Забыли пароль?', 'sidebar-login' ),
    ...
    public function widget( $args, $instance ) {
		...
		// Logged out user
			...
			$login_form_args = apply_filters( 'sidebar_login_widget_form_args', array(
		        ...
		        'label_username' 	=> __( 'Пользователь', 'sidebar-login' ),
		        'label_password' 	=> __( 'Пароль', 'sidebar-login' ),
		        'label_remember' 	=> __( 'Запомнить меня', 'sidebar-login' ),
		        'label_log_in' 		=> __( 'Войти &rarr;', 'sidebar-login' ),
